...
void enter_supervisor_mode_and_setup_mmu(void) {
    //print_str("Entering supervisor mode...\n");
    ...
    print_str("Benchmark Initialized!\n");
    // BLOCK RUNS ERRORS12 ERRORS13 ERRORS23 ERRORS CYCLES

    //print_str("BLOCK\tRUNS\tCYCLES\tERRORS12\tERRORS13\tERRORS23\tERRORS\n");
    ...
    init(); // inicializa matriz
    // test(); // no need?? since 'fill' multiplies already the golden matrix
    fill(); // rellena el golden    
    ...
    while (1) { // BLOQUES (variable dependiendo del tiempo de vida del programa)
        errors = 0;
        unmitigated = 0;
    	TMR_ERROR_CNT=0;
	...
	start_timer(); 
        ...
        if (run_counter >= MAX_RUNS || errors != 0 || unmitigated != 0) { // RUNS
            ...
            if (errors != 0 || unmitigated != 0) {
                init();
                // test_TMR_1(); // no need??
                fill(); // matriz cambia entre bloques cuando hay algun error
            }
...
int main(void) {
    ...
    //print_str("D1 NEZHA Cache-Enabled Matrix Multiplication Benchmark\n");
