...
//			errs() << "original function call: " << *u << "\n";
			...
			if (ConstantArray* ca = dyn_cast<ConstantArray>(u)) {
			...
			} else if (CallBase* callBase = dyn_cast<CallBase>(u)) {
				if (CallInst* callInst = dyn_cast<CallInst>(callBase)) {
				...
				} else if (InvokeInst* invInst = dyn_cast<InvokeInst>(callBase)) {
				...
				invInst->eraseFromParent();
					...
					// Handle other call instruction types (e.g., CallBrInst)
					if (verboseFlag) {
						errs() << "Warning: Unsupported call instruction type: " << *callBase << "\n";
					}
				}
			} else {
				// Not a call instruction at all
				if (verboseFlag) {
					errs() << "Warning: Expected call instruction but got: " << *u << "\n";
				}
			}
