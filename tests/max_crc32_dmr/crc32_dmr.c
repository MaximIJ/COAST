...
uint32_t crc_result_dmr_1;
uint32_t crc_result_dmr_2;
uint32_t crc_result;
uint32_t golden_crc;
...
uint32_t compute_crc32_dmr(uint8_t *data, size_t len) {
...
// if theres only one difference in the duplications,
// calculate one more time both CRC
uint32_t compute_crc32(uint8_t *data, size_t len) {
    // Compute CRC32 on duplicate arrays
    crc_result_dmr_1 = compute_crc32_dmr(data, len);
    crc_result_dmr_2 = compute_crc32_dmr(data, len);

    int diff = 0;

    if (crc_result_dmr_1 != crc_result_dmr_2) {
        diff = 1;
    }
    else {
        crc_result = crc_result_dmr_1;
    }

    // If different return the new run (crc_result)
    if (diff) {
        crc_result = compute_crc32_dmr(data, len);
    }
