...
unsigned char hashGlbl_dmr_1[32];
unsigned char hashGlbl_dmr_2[32];

void sha256_transform_dmr(uint32_t ctx_state[], unsigned char data[]) {
...
void sha256_hash_dmr(unsigned char ctx_data[], uint32_t ctx_bitlen[],
    ...
    for (i = 0; i < len; ++i) {
        ...
        (ctx_datalen)++;
        if (ctx_datalen == 64) {
            sha256_transform_dmr(ctx_state, ctx_data);
...
// DMR wrapper - runs hash twice and compares
void sha256_hash(unsigned char ctx_data[], uint32_t ctx_bitlen[],
    ...
    // Compute hash twice
    sha256_hash_dmr(ctx_data, ctx_bitlen, ctx_state, data, len, hashGlbl_dmr_1);
    sha256_hash_dmr(ctx_data, ctx_bitlen, ctx_state, data, len, hashGlbl_dmr_2);

    // Compare results
    for (i = 0; i < 32; i++) {
        if (hashGlbl_dmr_1[i] != hashGlbl_dmr_2[i]) {
            diff = 1;
            break;
        }
    }

    // If different, run a third time
    if (diff) {
        sha256_hash_dmr(ctx_data, ctx_bitlen, ctx_state, data, len, hash);
        local_errors++;
    } else {
        // Copy result
        for (i = 0; i < 32; i++) {
            hash[i] = hashGlbl_dmr_1[i];
        }
    }
}
