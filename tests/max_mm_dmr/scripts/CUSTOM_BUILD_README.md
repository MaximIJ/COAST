...
This directory contains scripts to build the `matrixMul_tmr` test with your custom RISC-V compiler located at:
```
...
**Usage:**
```bash
# From the matrixMul_tmr directory
./scripts/build_riscv.sh

# Or from anywhere
cd tests/matrixMul_tmr && ./scripts/build_riscv.sh
```
...
4. Creates `matrixMul_tmr_custom` executable
5. Backs up the original executable as `matrixMul_tmr.backup`

**Output files:**
- `matrixMul_tmr.o` - Object file
- `matrixMul_tmr.opt.ll` - Optimized LLVM IR
- `matrixMul_tmr_custom.s` - **Assembly file with custom compiler**
- `matrixMul_tmr_custom` - RISC-V executable with custom compiler
- `matrixMul_tmr.backup` - Original executable backup
...
# Or from anywhere
cd tests/matrixMul_tmr && ./scripts/build_custom.sh
```
...
4. Creates `matrixMul_tmr` executable directly

**Output files:**
- `matrixMul_tmr.o` - Object file
- `matrixMul_tmr.opt.ll` - Optimized LLVM IR  
- `matrixMul_tmr_custom.s` - **Assembly file with custom compiler**
- `matrixMul_tmr` - RISC-V executable with custom compiler
...
The generated assembly file (`matrixMul_tmr_custom.s`) contains:
- RISC-V assembly instructions
- Function definitions (init, test, fill, check, etc.)
