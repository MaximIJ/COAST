...
unsigned char hashGlbl_tmr_1[32];
unsigned char hashGlbl_tmr_2[32];
unsigned char hashGlbl_tmr_3[32];

void sha256_transform_tmr(uint32_t ctx_state[], unsigned char data[]) {
...
void sha256_hash_tmr_1(unsigned char ctx_data[], uint32_t ctx_bitlen[],
    ...
    for (i = 0; i < len; ++i) {
        ...
        (ctx_datalen)++;
        if (ctx_datalen == 64) {
            sha256_transform_tmr(ctx_state, ctx_data);
...
void sha256_hash_tmr_2(unsigned char ctx_data[], uint32_t ctx_bitlen[],
                 uint32_t ctx_state[], unsigned char data[],
                 uint32_t len, unsigned char hash[]) {
...
void sha256_hash_tmr_3(unsigned char ctx_data[], uint32_t ctx_bitlen[],
                 uint32_t ctx_state[], unsigned char data[],
                 uint32_t len, unsigned char hash[]) {
...
// Element-wise voter for hash arrays
void custom_Voter(unsigned char *result) {
    int i;
    for (i = 0; i < 32; i++) {
        if (hashGlbl_tmr_1[i] == hashGlbl_tmr_2[i] 
            && hashGlbl_tmr_1[i] == hashGlbl_tmr_3[i]) { // all agree
            result[i] = hashGlbl_tmr_1[i];
        } else if (hashGlbl_tmr_2[i] == hashGlbl_tmr_3[i]) { // error in 1
            result[i] = hashGlbl_tmr_2[i];
...
// TMR wrapper - runs hash three times and votes
void sha256_hash(unsigned char ctx_data[], uint32_t ctx_bitlen[],
                 uint32_t ctx_state[], unsigned char data[],
                 uint32_t len, unsigned char hash[]) {
    
    // Compute hash three times
    sha256_hash_tmr_1(ctx_data, ctx_bitlen, ctx_state, data, len, hashGlbl_tmr_1);
    sha256_hash_tmr_2(ctx_data, ctx_bitlen, ctx_state, data, len, hashGlbl_tmr_2);
    sha256_hash_tmr_3(ctx_data, ctx_bitlen, ctx_state, data, len, hashGlbl_tmr_3);
...
void check_tmr(void) {
    ...
    for (i = 0; i < 32; i++) {
        ...
        if (hashGlbl_tmr_1[i] != hashGlbl_tmr_3[i]) {
            errors_13++;
        }
        if (hashGlbl_tmr_2[i] != hashGlbl_tmr_3[i]) {
            errors_23++;
        }
    }
