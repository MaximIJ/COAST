...
This directory contains scripts to build the `quicksort_tmr` test with your custom RISC-V compiler located at:
```
...
**Usage:**
```bash
# From the quicksort_tmr directory
./scripts/build_riscv.sh

# Or from anywhere
cd tests/max_quicksort_tmr && ./scripts/build_riscv.sh
```

**Output files:**
- `quicksort_tmr.o` - Object file
- `quicksort_tmr.opt.ll` - Optimized LLVM IR
- `quicksort_tmr_custom.s` - **Assembly file with TMR variables**
- `quicksort_tmr_custom` - RISC-V executable with custom compiler
